/*
Each node in the command's AST must contain information about the input token it represents
The node must also contain pointers to its child nodes(if the node is a root node), as well
as its sibling nodes(if the node is a child node). struct node_s will represent nodes in
our AST.
*/
...
struct node_s
{
    enum   node_type_e type;    /* type of this node- root and child(command name and
				arguments*/
